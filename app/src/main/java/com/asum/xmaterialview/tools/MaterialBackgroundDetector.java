...
/**
 * The detector handle the touch event, and display a circle mask on your canvas. Created by kifile on 15-1-4.
 */
public class MaterialBackgroundDetector {
	...
	private void setColor(int color) {
		if (mColor != color) {
			...
			mColor = color;
			setAlpha(XMViewConfig.RIPPLE_ALPHA);
		}
	}
	...
	private int computeFocusColor(int color, int alpha) {
		return ColorUtils.getColorAtAlpha(color, 0);
	}
	...
	private void startShadowAnimation(MotionEvent event) {
		...
		int mDuration = XMViewConfig.DURATION_SLOW;
	...
	private void cancelShadowAnimation() {
		...
		int duration = (int) (XMViewConfig.DURATION_FAST * (mViewRadius - mRadius) / mViewRadius);
		if (duration > 0) {
	...
	private void showAlphaAnimation() {
		ObjectAnimator alphaAnimator = ObjectAnimator.ofInt(this, "alpha", XMViewConfig.RIPPLE_ALPHA, 0);
		alphaAnimator.setDuration(XMViewConfig.INTERVAL);
	...
	public void setRadius(float radius) {
		...
		if (mHasDrawMask) {
			...
			mView.invalidate((int) (mCenterX - mRadius), (int) (mCenterY - mRadius), (int) (mCenterX + mRadius), (int) (mCenterY + mRadius));
		} else {
			mView.invalidate();
		}
	}
	...
	/**
	 * It only happens when the fingers up. See also {@link #handlePerformLongClick()}
	 *
	...
	/**
	 * It only happens when the fingers up. See also {@link #handlePerformClick()} ()}
	 *
	 * @return whether it is been handled.
	 */
